...
视为一个航程；清洗后写入独立文件：
  <输入主名>-j001-cleaned-flight_info.json、-j002-...（每条为航班对象数组，字段名为中文：成人票面价、出发机场、航班号等）

用法:
  python QF_清洗.py              # 交互输入路径
  python QF_清洗.py 源.json [--no-route-filter]
...
def build_flight_info_qf(
    ...
    # 中文字段名与业务参考一致（与英文 aduPrice/depAirport 等一一对应）
    return {
        "成人票面价": adu_price,
